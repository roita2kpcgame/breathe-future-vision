...
// Function to fetch real pollution data from WAQI API
export const fetchPollutionData = async (): Promise<City[]> => {
  ...
  for (const city of initialCitiesData) {
    try {
      // Try to fetch data for the city
      const response = await fetch(`https://api.waqi.info/feed/${city.name.toLowerCase()}/?token=${apiToken}`);
      const data = await response.json();
      
      if (data.status === 'ok' && data.data) {
        const pm25 = data.data.iaqi?.pm25?.v || city.pm25;
        const pm10 = data.data.iaqi?.pm10?.v || city.pm10;
      ...
      console.log(`Failed to fetch data for ${city.name}, using default data`);
      updatedCities.push({
        ...city,
        position: cityPositions[city.name as keyof typeof cityPositions]
      });
